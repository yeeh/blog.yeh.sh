...
传统旁路由也接在主路由后的交换机上，但需要代理的客户端，要手动把 DNS 和默认网关改为软路由的 LAN IP。软路由的上游网关仍是主路由。

![传统旁路由：软路由接入交换机](/uploads/2026/09-07-traditional-bypass-router.svg)

这样做不需要改变主路由配置，但有两个代价：每台需要代理的设备都要单独设置；并且这些设备的直连流量和代理流量都会先经过软路由。也可以准备两个 Wi-Fi，一个下发软路由的 DNS 和网关，另一个保持主路由设置，通过切换 Wi-Fi 改变流量路径。
